...
import { fetchAllBlogs, addComment } from "../hooks/blog";
...
const BlogList = () => {
  ...
  const displayedBlogs = showMyPosts
    ? blogs.filter((b) => b.authorID === user?.userID)
    : blogs;
    ...
    <div className="bloglist-wrapper">
      <header className="bloglist-header">
        <nav className="navbar border-bottom sticky-top bg-white py-3">
          <div className="container-xl d-flex justify-content-between align-items-center px-4">
            ...
            >
              <span className="fw-bold fs-4 bolder tracking-tighter m-0">
                BLOG.
              </span>
            </div>
      ...
      <div className="blog-feed">
        {displayedBlogs.length === 0 && (
          <p className="text-center text-muted mt-5">No blogs found.</p>
        )}

        {displayedBlogs.map((blog) => {
          const comments = blog.comments ?? [];
          const commentsToShow = showAllComments[blog.id]
            ? comments
            : comments.slice(0, 3);
          ...
          return (
            <div key={blog.id} className="blog-card shadow-sm">
                ...
                <div className="dropdown">
                  ...
                  <div className="dropdown-content">
                    <p
                      ...
                      onClick={() => navigate(`/bloglist/blog/${blog.id}`)}
                    >
                      View Post
                    </p>
                    {blog.authorID === user?.userID && (
                      <p
                        className="dropdown-item"
                        onClick={() =>
                          navigate(`/bloglist/edit-blog/${blog.id}`, {
                            state: { blog },
                          })
                        }
                      >
              ...
              <div className="blog-actions d-flex justify-content-evenly mt-3 px-1">
                <span
                  className={`d-flex align-items-center gap-1 ${likedBlogs[blog.id] ? "text-primary fw-semibold" : "text-muted"}`}
                ...
                <span
                  ...
                  style={{ cursor: "pointer" }}
                  onClick={() => {
                    commentInputRefs.current[blog.id]?.focus();
                  }}
                >
              ...
              <div className="blog-comments">
                      ...
                      {c.imagePath && (
                        <div className="ms-5 mb-1">
                          <img
                            ...
                            onClick={() => openImageModal(c.imagePath!)}
                          />
                        </div>
                      )}
                ...
                {comments.length > 3 && (
                  <p
                    className="text-primary small ms-5 mb-2"
                    style={{ cursor: "pointer" }}
                    onClick={() =>
                  ...
                  <div className="add-comment d-flex align-items-center gap-2 mt-2">
                    ...
                    <input
                      ref={(el: HTMLInputElement | null) => {
                        commentInputRefs.current[blog.id] = el;
                      }}
                      type="text"
                    ...
                    <Button
                      onClick={() =>
                        document
                          .getElementById(`comment-file-${blog.id}`)
                          ?.click()
                      }
                      colorVariant="light"
        ...
        {total > ITEMS_PER_PAGE && (
          <div className="pagination mt-4 d-flex justify-content-center gap-2">
            {Array.from({ length: Math.ceil(total / ITEMS_PER_PAGE) }).map(
              (_, idx) => (
                <Button
                  key={idx}
